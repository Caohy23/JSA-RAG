...
class CosineScheduler(torch.optim.lr_scheduler.LambdaLR):
    def __init__(self, optimizer, warmup, total, ratio=0.01, last_epoch=-1):
        self.warmup = warmup
        self.total = total
        self.ratio = ratio
    ...
    def lr_lambda(self, step):
        ...
        elif self.warmup < step < self.total*0.5:
            s = float(step - self.warmup) / (self.total*0.5 - self.warmup)
...
def set_optim(opt, model):
    ...
    base_optim_class = AdamW
    optim_args = {"weight_decay": opt.weight_decay, "betas": (0.9, opt.beta2), "eps": opt.epsilon}
    ret_optim_args = {"weight_decay": opt.weight_decay, "betas": (0.9, opt.beta2), "eps": opt.ret_epsilon}

    # 检查是否需要使用 OSS
    # if opt.is_distributed and opt.shard_optim:
    #     optim_class = OSS
    #     optim_args["optim"] = base_optim_class
    #     ret_optim_args["optim"] = base_optim_class
    # else:
    optim_class = base_optim_class

    def get_retriever_params(model, opt):
        retriever_params = list(model.retriever.parameters())
        if opt.gold_score_mode in ['vrag', 'jsa'] and not opt.simplify_JSA:
            if opt.decouple_encoder:
                retriever_params += list(model.post_retriever.query_retriever.parameters())
            else:
                retriever_params += list(model.post_retriever.parameters())
        return retriever_params

    if not opt.separate_learning_rates:
        all_params = list(model.generator.parameters())
        if opt.train_retriever:
            all_params = get_retriever_params(model, opt)
        optimizer = optim_class(params=model.parameters(), lr=opt.lr, **optim_args)
        #print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
        retr_optimizer = None
    else:
        all_params = list(model.generator.parameters())
        if not opt.query_side_retriever_training:
            all_params += list(model.post_retriever.query_retriever.parameters())
            all_params += list(model.retriever.query_retriever.parameters())
        optimizer = optim_class(params=all_params, lr=opt.lr, **optim_args)
        if opt.train_retriever:
            if not opt.query_side_retriever_training:
                retriever_params = list(model.retriever.passage_retriever.parameters())
            else:
                retriever_params = get_retriever_params(model, opt)
            retr_optimizer = optim_class(params=retriever_params, lr=opt.lr_retriever, **ret_optim_args)
...
#         ret_optim_args["optim"] = optim_class
...
#                 else:
#                     retriever_params += list(model.post_retriever.parameters())
#             all_params += retriever_params
#         optimizer = optim_class(params=all_params, lr=opt.lr, **optim_args)
#         retr_optimizer = None
#     else:
#         optimizer = optim_class(params=model.generator.parameters(), lr=opt.lr, **optim_args)
#         if opt.train_retriever:
#             retriever_params = list(model.retriever.parameters())
#             if opt.gold_score_mode in ['vrag', 'jsa'] and not opt.simplify_JSA:
#                 if opt.decouple_encoder:
...
def compute_grad_stats(model):
    with torch.no_grad():
        stats = []
        for name, p in get_unwrapped_model_if_wrapped(model).generator.named_parameters():
